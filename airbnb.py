...
if selected == "Home":         
    ...
    with coll3: 
        st.image("C:\Airbnb\logo.jpg",width=465)  
    ...
    with coll6:         
        st.image("C:/Airbnb/dom.jpg",width=465) 
    ...
    df=pd.read_csv("C:\Airbnb\Airbnb.csv")
    ...
    with col2: 
        ...
        df1= df1= df.query(query).groupby(["Transit"]).size().reset_index(name="count").sort_values(by='count',ascending=False)[:10]
        fig = px.bar(df1,
    ...
    st.markdown('''I Created this Internship Data Analysis Project Using "Python" to perform Data Cleansing, Understand Dataset, 
